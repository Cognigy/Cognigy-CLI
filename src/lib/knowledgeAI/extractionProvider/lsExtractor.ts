...
type LSDocumentLoaderTypes = 'text' | 'pdf' | 'docx' | 'csv' | 'json' | 'jsonl' | 'epub' | 'srt' | 'md' | 'cheerio' | 'playwright';
...
export const lsExtractor = async (type: LSDocumentLoaderTypes, options: IExtractOptions) => {
    let documentLoader;
    switch (type) {
        case "text":
        ...
        case "pdf":
            ...
            documentLoader = new PDFLoader(options.inputFile, options.additionalParameters || { splitPages: false });
            break;
        ...
        case "csv":
            ...
            documentLoader = new CSVLoader(options.inputFile, options.additionalParameters);
            break;
        ...
        case "epub":
            ...
            documentLoader = new EPubLoader(options.inputFile, options.additionalParameters || { splitChapters: true });
            break;
        ...
        case "json":
            ...
            documentLoader = new JSONLoader(options.inputFile, options.additionalParameters);
            break;

        case "jsonl":
            // possible config: pointer
            // https://js.langchain.com/docs/modules/indexes/document_loaders/examples/file_loaders/jsonlines
            documentLoader = new JSONLinesLoader(options.inputFile, options.additionalParameters);
            break;
        ...
        case 'cheerio':
            ...
            if (options.url) {
                documentLoader = new CheerioWebBaseLoader(options.url, options.additionalParameters);
            } else { // needed for local html retrieval
                documentLoader = new TextLoader(options.inputFile);
            }
            break;
        ...
        case "playwright":
            ...
            if (options.url) {
                documentLoader = new PlaywrightWebBaseLoader(options.url, options.additionalParameters);
            } else { // needed for local html retrieval
                documentLoader = new TextLoader(options.inputFile);
            }
            break;
        ...
        if (type === 'playwright' || type === 'cheerio') {
            doc.pageContent = convert(doc.pageContent, options.additionalParameters || {
                ...
                selectors: [
                    ...
                    { selector: 'pre', options: { leadingLineBreaks: 0, trailingLineBreaks: 1 } }
                ]
            });
        }

        doc.pageContent = doc?.pageContent?.replace(/(\s){2,}/g, ' ') // replace multiple spaces with single space
