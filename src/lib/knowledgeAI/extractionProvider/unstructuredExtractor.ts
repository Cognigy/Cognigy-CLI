 ...
 * @returns 
 */
async function postData(fileName: string): Promise<IUnstructuredResponseElement[] | null> {
    ...
    try {
        ...
        console.log(`Error when extracting content from ${fileName} via API: ${error.message}`);
        return null;
    }
}
 ...
 * @returns 
 */
async function cleanData(data: IUnstructuredResponseElement[], options?: IExtractOptions): Promise<string> {
    let result = "";

    data.forEach((item: IUnstructuredResponseElement) => {
        if (item.type === "NarrativeText" && (!options?.excludeString || !item.text.toLowerCase().includes(options?.excludeString?.toLocaleLowerCase()))) {
            result += `${item.text}\n\n`;
        }
    });
 ...
 */
export const unstructuredExtractor = async (options: IExtractOptions): Promise<string | null> => {
