 ...
 * @returns 
 */
function extractArticles(jsonArray: DiffbotArticlePage[], excludeString?: string): string {
    let result = '';
    jsonArray.forEach((article) => {
        
        if (article.text) {
            const paragraphs = splitParagraphsByTokenLength(article.title, article.text, `[url: ${article.pageUrl}]`);

            paragraphs.forEach((paragraph, index) => {
                result +=  ((!excludeString || (excludeString && paragraph.includes(excludeString)))) ? paragraph + "\n" : "";
 ...
 * @returns 
 */
function extractFAQs(jsonArray: DiffbotFAQPage[], excludeString?: string): string {
    let result = '';
    jsonArray.forEach((faqPage, index) => {
        try {
            const faqs = faqPage.faqs;
            faqs.forEach((faq: any) => {
                result += (faq.q && faq.a && (!excludeString || (excludeString && !faq.q.includes(excludeString) && !faq.a.includes(excludeString)))) ? `${faq.q}\n${faq.a}\n[url: ${faqPage.pageUrl}]\n` : "";
