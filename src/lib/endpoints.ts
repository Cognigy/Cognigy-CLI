 ...
 */
export const cloneEndpoints = async (
  ...
  for (let endpoint of endpoints.items) {
    ...
    fs.writeFileSync(
      individualEndpointDir + '/config.json',
      JSON.stringify(endpointDetail, undefined, 4)
    );
    fs.writeFileSync(
      individualEndpointDir + '/transformer.ts',
      endpointDetail.transformer.transformer
    );
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const pullEndpoint = async (
  endpointName: string,
  availableProgress: number
): Promise<void> => {
  ...
  // write files to disk
  fs.writeFileSync(
    endpointDir + '/config.json',
    JSON.stringify(endpointDetail, undefined, 4)
  );
  fs.writeFileSync(
    endpointDir + '/transformer.ts',
    endpointDetail.transformer.transformer
  );
  addToProgressBar(70);
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const restoreEndpoints = async (
  availableProgress: number
): Promise<void> => {
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const pushEndpoint = async (
  endpointName: string,
  availableProgress: number
): Promise<void> => {
  const endpointDir = CONFIG.agentDir + '/endpoints/' + endpointName;
  ...
  if (fs.existsSync(endpointDir + '/config.json')) {
    ...
    try {
      ...
      const endpointConfig = JSON.parse(
        fs.readFileSync(endpointDir + '/config.json').toString()
      );
      ...
      try {
        const transformer = fs
          .readFileSync(endpointDir + '/transformer.ts')
          .toString();
      ...
      console.error(
        `Error when updating Endpoint ${endpointName} on Cognigy.AI: ${err.message}.\nAborting...`
      );
      process.exit(0);
    }
 ...
 */
export const diffEndpoints = async (
  endpointName: string,
  mode: string = 'full'
): Promise<void> => {
  try {
    ...
    if (['full'].indexOf(mode) === -1) {
      console.log(
        `Selected mode not supported for Endpoints. Supported modes:\n\n- full\n`
      );
      process.exit(0);
    }

    const spinner = new Spinner(
      `Comparing ${chalk.green('local')} and ${chalk.red('remote')} Endpoint resource ${endpointName}... %s`
    );
    ...
    if (
      !fs.existsSync(endpointDir + '/' + endpointName) ||
      !fs.existsSync(endpointDir + '/' + endpointName + '/config.json')
    ) {
      spinner.stop();
      console.log(
    ...
    const localConfig = JSON.parse(
      fs
        .readFileSync(endpointDir + '/' + endpointName + '/config.json')
        .toString()
    );
    ...
    try {
      const transformer = fs
        .readFileSync(endpointDir + '/' + endpointName + '/transformer.ts')
        .toString();
