...
export const runPlaybooks = async (playbookFile: string): Promise<void> => {
  ...
  const playbooksArray: Playbook[] = JSON.parse(
    fs.readFileSync(playbooksFile).toString()
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(playbookRunResults.playbookRunStatuses, null, 2)
  );

  // Display the runstatus
  if (playbookRunResults.status === 'successful') {
    console.log(
      `SUCCESS - all ${playbookRunResults.playbookRunStatuses.length} playbooks were executed successfully.`
    );
    process.exit(0);
    ...
    console.log(
      `TIMEOUT - couldn't process all playbooks within the allotted time. You can adjust the timeout in the config.json file.`
    );
    process.exit(2);
  }
};
 ...
 */
async function checkPlaybookRuns(
  scheduledPlaybookRuns: PlaybookRun[],
  counter: number,
  realResolve?: Function
): Promise<{ status: string; playbookRunStatuses: any }> {
  counter++;
  return new Promise((resolve) => {
    ...
    setTimeout(async () => {
      ...
      if (playbookRunStatuses.every((pbRun) => pbRun.status === 'successful')) {
        ...
        playbookRunStatuses.every(
          (pbRun) => pbRun.status === 'successful' || pbRun.status === 'failed'
        )
      ) {
        resolver({
          status: 'failure',
