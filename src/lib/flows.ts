...
import translateFlowNode, {
  translateIntentExampleSentence,
  translateSayNode,
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const pullFlow = async (
  flowName: string,
  availableProgress: number,
  _flows?: Pick<IIndexFlowsRestReturnValue_2_0, 'items' | 'total'>
): Promise<void> => {
  // The base directory for Flows
  ...
  for (let locale of locales) {
    ...
    fs.writeFileSync(
      flowDir + '/config.json',
      JSON.stringify(flow, undefined, 4)
    );
    ...
    fs.writeFileSync(
      localeDir + '/chart.json',
      JSON.stringify(chart, undefined, 4)
    );
    ...
    fs.writeFileSync(
      localeDir + '/intents.json',
      JSON.stringify(flowIntents, undefined, 4)
    );
  }
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const restoreFlows = async (
  availableProgress: number
): Promise<void> => {
...
export const pushFlow = async (
  ...
  if (
    fs.existsSync(flowDir + '/config.json') &&
    fs.existsSync(flowsDir + '/locales.json')
  ) {
    const locales = JSON.parse(
      fs.readFileSync(flowsDir + '/locales.json').toString()
    );
    ...
    for (let locale of locales) {
      ...
      try {
        const flowConfig = JSON.parse(
            fs.readFileSync(flowDir + '/config.json').toString()
          ),
          flowChart = JSON.parse(
            fs
              .readFileSync(flowDir + '/' + locale.name + '/chart.json')
              .toString()
          ),
          flowIntents: IIntent[] = JSON.parse(
            fs
              .readFileSync(flowDir + '/' + locale.name + '/intents.json')
              .toString()
          );
        ...
        try {
          ...
          console.error(
            `Error when updating Flow ${flowName} on Cognigy.AI: ${err.message}.\nAborting...`
          );
          process.exit(0);
        }

        const progressPerNode =
          availableProgress / 2 / locales.length / flowChart.nodes.length;
        ...
        if (flowIntents.length > 0) {
          ...
          form.append(
            'file',
            fs.createReadStream(flowDir + '/' + locale.name + '/intents.json')
          );
...
export const diffFlows = async (
  ...
  if (['full', 'node'].indexOf(mode) === -1) {
    console.log(
      `Selected mode not supported. Supported modes:\n\n- full\n- node\n`
    );
    return;
  }

  const spinner = new Spinner(
    `Comparing ${chalk.green('local')} and ${chalk.red('remote')} Flow resource ${chalk.blueBright(flowName)}... %s`
  );
  ...
  const locales: ILocaleIndexItem_2_0[] = JSON.parse(
    fs.readFileSync(flowsDir + '/locales.json').toString()
  );
  ...
  for (let locale of locales) {
    ...
    if (
      !fs.existsSync(flowDir) ||
      !fs.existsSync(flowDir + '/' + locale.name + '/chart.json') ||
      !fs.existsSync(flowDir + '/config.json')
    ) {
      spinner.stop();
      console.error(
    ...
    const localChart = JSON.parse(
        fs
          .readFileSync(
            flowsDir + '/' + flowName + '/' + locale.name + '/chart.json'
          )
          .toString()
      ),
      localConfig = JSON.parse(
        fs.readFileSync(flowsDir + '/' + flowName + '/config.json').toString()
      );
...
export const trainFlow = async (
  ...
  for (let locale of locales) {
    const spinner = new Spinner(
      `Training intents for locale ${chalk.yellow(locale.name)} ... %s`
    );
    spinner.setSpinnerString('|/-\\');
    spinner.start();
 ...
 */
export const translateFlow = async (
  flowName: string,
  options: ITranslateFlowOptions
): Promise<void> => {
  ...
  try {
    const flowConfig = JSON.parse(
      ...
      flowChart = JSON.parse(
        fs.readFileSync(flowDir + '/' + localeName + '/chart.json').toString()
      );

    const targetLocale = (await pullLocales()).find(
      (locale) => locale.name === localeName
    );

    const flowIntents = (
      await indexAll(CognigyClient.indexIntents)({
        flowId: flowConfig._id,
        ...
        try {
          if (localeReference === targetLocale._id) {
            if (['say', 'question', 'optionalQuestion'].indexOf(type) > -1) {
              const flowNode = await translateFlowNode(
                node,
                toLanguage,
                translator,
                apiKey
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const exportFlowCSV = async (
  flowName: string,
  availableProgress: number
): Promise<void> => {
  const flowsDir = CONFIG.agentDir + '/flows';
    ...
    for (let locale of locales) {
      ...
      try {
        ...
        const csvWriterContent = createArrayCsvWriter({
          ...
          header: ['id', 'label', 'type', 'subtype', 'localized', 'content'],
        });
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const importFlowCSV = async (
  flowName: string,
  availableProgress: number
): Promise<void> => {
  const flowsDir = CONFIG.agentDir + '/flows';
  ...
  if (
    ...
    for (let locale of locales) {
      // chart and config exist for this flow, proceed
      try {
        if (!fs.existsSync(flowDir + '/' + locale.name + '/content.csv')) break;
                  ...
                  case 'say':
                    switch (node.config.say.type) {
                      case 'text':
                        ...
                        else node.config.say.text = [parsedContent];
                        break;
        ...
        fs.writeFileSync(
          flowDir + '/' + locale.name + '/chart.json',
          JSON.stringify(flowChart, undefined, 4)
        );
      } catch (err) {}
    }
  }
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const localizeFlow = async (
  flowName: string,
  availableProgress: number,
  options
): Promise<void> => {
  ...
  try {
    const primaryLocale = (await pullLocales()).find(
      (locale) => locale.primary === true
    );
    const targetLocale = (await pullLocales()).find(
      (locale) => locale.name === localeName
    );
    const sourceLocale = (await pullLocales()).find(
      (locale) => locale.name === sourceLocaleName
    );
    ...
    const flowConfig = JSON.parse(
      ...
      flowIntents: IIntent[] = JSON.parse(
        fs
          .readFileSync(flowDir + '/' + primaryLocale.name + '/intents.json')
          .toString()
      );
      ...
      for (let node of flowChart.nodes) {
        ...
        try {
          if (options.reverse) {
            ...
            if (localeReference !== targetLocale._id) {
              if (
                !onlyLocalizeContentNodes ||
                ['say', 'question', 'optionalQuestion'].indexOf(type) > -1
              ) {
 ...
 */
export const translateIntent = async (
  intent: any,
  flowId,
  targetLocale,
  toLanguage,
  ...
  if (intentData.data) {
    try {
      intentData.data = await translateSayNode(
        intentData.data,
        toLanguage,
        translator,
        apiKey
  ...
  if (
    flowIntentSentences &&
    flowIntentSentences.items &&
    flowIntentSentences.items.length > 0
  ) {
  ...
  try {
    for (let sentence of intentSentences) {
      ...
      sentence = await translateIntentExampleSentence(
        sentence,
        toLanguage,
        translator,
        apiKey
