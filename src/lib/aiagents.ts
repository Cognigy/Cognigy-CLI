 ...
 */
export const cloneAiAgents = async (progressIncrement: number = 100): Promise<void> => {
    ...
    for (const agent of aiAgents) {
        ...
        const fullAiAgent = await CognigyClient.readAiAgent({ aiAgentId: agent._id });
...
export const pullAiAgent = async (resourceName?: string): Promise<void> => {
    ...
    if (resourceName) {
        ...
        const fullAiAgent = await CognigyClient.readAiAgent({ aiAgentId: aiAgent._id });
 ...
 */
export const pushAiAgent = async (resourceName: string, availableProgress: number = 100): Promise<void> => {
    const agentDir = getAgentDir(resourceName);
    ...
    try {
        ...
        console.error(error);
        throw new Error(`Failed to update AI Agent '${resourceName}' in Cognigy.AI`);
    }
 ...
 * @param availableProgress How much of the progress bar can be filled by this process
 */
export const restoreAiAgents = async (availableProgress: number): Promise<void> => {
 ...
 */
export const diffAiAgents = async (aiAgentName: string, mode: string = 'full'): Promise<void> => {
    try {
        // check if a valid mode was selected
        if (['full'].indexOf(mode) === -1) {
            console.log(`Selected mode not supported for AI Agents. Supported modes:\n\n- full\n`);
            process.exit(0);
        }

        const spinner = new Spinner(`Comparing ${chalk.green('local')} and ${chalk.red('remote')} AI Agent resource ${aiAgentName}... %s`);
