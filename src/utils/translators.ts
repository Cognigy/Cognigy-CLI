...
export const translateIntentExampleSentence = async (
  sentence: ISentence,
  language: string,
  ...
  try {
    ...
    switch (type) {
      case 'optionalQuestion':
        flowNode.config.say = await translateSayNode(
          config.say,
          targetLanguage,
          translator,
          apiKey
      ...
      case 'question':
        ...
        if (flowNode.config.validationMessage)
          flowNode.config.validationMessage = await translate(
            flowNode.config.validationMessage,
            targetLanguage,
            translator,
            apiKey
          );
        if (flowNode.config.datepicker_eventName)
          flowNode.config.datepicker_eventName = await translate(
            flowNode.config.datepicker_eventName,
            targetLanguage,
            translator,
            apiKey
          );
        if (flowNode.config.datepicker_openPickerButtonText)
          flowNode.config.datepicker_openPickerButtonText = await translate(
            flowNode.config.datepicker_openPickerButtonText,
            targetLanguage,
            translator,
            apiKey
          );
        if (flowNode.config.datepicker_cancelButtonText)
          flowNode.config.datepicker_cancelButtonText = await translate(
            flowNode.config.datepicker_cancelButtonText,
            targetLanguage,
            translator,
            apiKey
          );
        if (flowNode.config.datepicker_submitButtonText)
          flowNode.config.datepicker_submitButtonText = await translate(
            flowNode.config.datepicker_submitButtonText,
            targetLanguage,
            translator,
            apiKey
          );
...
export async function translateSayNode(data, language, translator, apikey) {
  ...
  if (data.text) {
    ...
    for (let sentence of data.text) {
      ...
      // Translate the text
      data.text[index] = await translate(
        data.text[index],
        language,
        translator,
        apikey
  ...
  if (data?._cognigy?._default?._list?.items) {
    ...
    if (data._cognigy._default._list.fallbackText) {
      data._cognigy._default._list.fallbackText = await translate(
        data._cognigy._default._list.fallbackText,
        language,
        translator,
        apikey
      );
    ...
    for (let item of data._cognigy._default._list.items) {
      ...
      data._cognigy._default._list.items[index].title = await translate(
        data._cognigy._default._list.items[index].title,
        language,
        translator,
        apikey
      );
      data._cognigy._default._list.items[index].subtitle = await translate(
        data._cognigy._default._list.items[index].subtitle,
        language,
        translator,
        apikey
      );
      ...
      if (data?._cognigy?._default?._list?.items[index]?.buttons) {
        ...
        for (let button of data._cognigy._default._list.items[index].buttons) {
          let buttonIndex =
            data._cognigy._default._list.items[index].buttons.indexOf(button);

          // translate the button title
          data._cognigy._default._list.items[index].buttons[buttonIndex].title =
            await translate(
              data._cognigy._default._list.items[index].buttons[buttonIndex]
                .title,
              language,
              translator,
              apikey
          ...
          if (button.type === 'postback') {
            ...
            data._cognigy._default._list.items[index].buttons[
              buttonIndex
            ].payload = await translate(
              data._cognigy._default._list.items[index].buttons[buttonIndex]
                .payload,
              language,
              translator,
    ...
    if (data._cognigy?._default?._list?.button) {
      data._cognigy._default._list.button.title = await translate(
        data._cognigy?._default?._list?.button.title,
        language,
        translator,
        apikey
      );
      ...
      if (data._cognigy?._default?._list?.button.type === 'postback') {
        // Translate button title
        data._cognigy._default._list.button.payload = await translate(
          data._cognigy._default._list.button.payload,
          language,
          translator,
          apikey
        );
    ...
    for (let item of data._data._cognigy._default._list.items) {
      ...
      data._data._cognigy._default._list.items[index].title = await translate(
        data._data._cognigy._default._list.items[index].title,
        language,
        translator,
        apikey
      );
      data._data._cognigy._default._list.items[index].subtitle =
        await translate(
          data._data._cognigy._default._list.items[index].subtitle,
          language,
          translator,
      ...
      if (data?._data._cognigy?._default?._list?.items[index]?.buttons) {
        // Loop through the buttons
        for (let button of data._data._cognigy._default._list.items[index]
          .buttons) {
          let buttonIndex =
            data._data._cognigy._default._list.items[index].buttons.indexOf(
              button
            );

          // translate the button title
          data._data._cognigy._default._list.items[index].buttons[
            buttonIndex
          ].title = await translate(
            data._data._cognigy._default._list.items[index].buttons[buttonIndex]
              .title,
            language,
            translator,
            apikey
    ...
    if (data._data._cognigy?._default?._list?.button) {
      data._data._cognigy._default._list.button.title = await translate(
        data._data._cognigy?._default?._list?.button.title,
        language,
        translator,
        apikey
      );
      ...
      if (data._data._cognigy?._default?._list?.button.type === 'postback') {
        // Translate button title
        data._data._cognigy._default._list.button.payload = await translate(
          data._data._cognigy._default._list.button.payload,
          language,
          translator,
          apikey
        );
  ...
  if (data?._cognigy?._default?._buttons?.buttons) {
    ...
    if (data._cognigy._default._buttons.fallbackText) {
      data._cognigy._default._buttons.fallbackText = await translate(
        data._cognigy._default._buttons.fallbackText,
        language,
        translator,
        apikey
      );
    ...
    for (let button of data._cognigy._default._buttons.buttons) {
      ...
      if (button.type === 'postback') {
        // Translate button title
        data._cognigy._default._buttons.buttons[index].title = await translate(
          data._cognigy._default._buttons.buttons[index].title,
          language,
          translator,
          apikey
        );
      ...
      data._cognigy._default._buttons.text = await translate(
        data._cognigy._default._buttons.text,
        language,
        translator,
        apikey
      );
    ...
    for (let button of data._data._cognigy._default._buttons.buttons) {
      ...
      if (button.type === 'postback') {
        // Translate button title
        data._data._cognigy._default._buttons.buttons[index].title =
          await translate(
            data._data._cognigy._default._buttons.buttons[index].title,
            language,
            translator,
      ...
      // Translate the text of the button message
      data._data._cognigy._default._buttons.text = await translate(
        data._data._cognigy._default._buttons.text,
        language,
        translator,
        apikey
      );
  ...
  if (data?._cognigy?._default?._quickReplies?.quickReplies) {
    ...
    if (data._cognigy._default._quickReplies.fallbackText) {
      data._cognigy._default._quickReplies.fallbackText = await translate(
        data._cognigy._default._quickReplies.fallbackText,
        language,
        translator,
        apikey
      );
    ...
    for (let quickReply of data._cognigy._default._quickReplies.quickReplies) {
      // Get the index of the current sentence in the list of sentences called 'text'
      let index =
        data._cognigy._default._quickReplies.quickReplies.indexOf(quickReply);

      // Check type of button
      if (quickReply.contentType === 'postback') {
        // Translate quick reply
        data._cognigy._default._quickReplies.quickReplies[index].title =
          await translate(
            data._cognigy._default._quickReplies.quickReplies[index].title,
            language,
            translator,
        ...
        data._cognigy._default._quickReplies.quickReplies[index].payload =
          await translate(
            data._cognigy._default._quickReplies.quickReplies[index].payload,
            language,
            translator,
      ...
      data._cognigy._default._quickReplies.text = await translate(
        data._cognigy._default._quickReplies.text,
        language,
        translator,
        apikey
      );
    ...
    // Loop through the quick replies
    for (let quickReply of data._data._cognigy._default._quickReplies
      .quickReplies) {
      // Get the index of the current sentence in the list of sentences called 'text'
      let index =
        data._data._cognigy._default._quickReplies.quickReplies.indexOf(
          quickReply
        );
      ...
      if (quickReply.contentType === 'postback') {
        // Translate quick reply
        data._data._cognigy._default._quickReplies.quickReplies[index].title =
          await translate(
            data._data._cognigy._default._quickReplies.quickReplies[index]
              .title,
            language,
            translator,
            apikey
          );
        data._data._cognigy._default._quickReplies.quickReplies[index].payload =
          await translate(
            data._data._cognigy._default._quickReplies.quickReplies[index]
              .payload,
            language,
      ...
      // Translate the text of the quick reply text
      data._data._cognigy._default._quickReplies.text = await translate(
        data._data._cognigy._default._quickReplies.text,
        language,
        translator,
        apikey
      );
  ...
  if (data?._cognigy?._default?._gallery) {
    ...
    if (
      data._cognigy._default._gallery.fallbackText &&
      data._cognigy._default._gallery.fallbackText !== ''
    ) {
      data._cognigy._default._gallery.fallbackText = await translate(
        data._cognigy._default._gallery.fallbackText,
        language,
        translator,
        apikey
    ...
    for (let item of data._cognigy._default._gallery.items) {
      ...
      item.subtitle = await translate(
        item.subtitle,
        language,
        translator,
        apikey
      );
      item.imageAltText = await translate(
        item.imageAltText,
        language,
        translator,
        apikey
      ...
      if (item.buttons && item.buttons.length > 0) {
        for (let button of item.buttons) {
          ...
          if (button.type === 'postback') {
            button.payload = await translate(
              button.payload,
              language,
              translator,
              apikey
    ...
    if (data?._data?._cognigy?._default?._gallery?.fallbackText !== '') {
      data._data._cognigy._default._gallery.fallbackText = await translate(
        data._data._cognigy._default._gallery.fallbackText,
        language,
        translator,
        apikey
      );
  ...
  if (data?._cognigy?._default?._image) {
    // Translate fallback text
    data._cognigy._default._image.fallbackText = await translate(
      data._cognigy._default._image.fallbackText,
      language,
      translator,
      apikey
    );
    // Translate alternative text
    data._cognigy._default._image.imageAltText = await translate(
      data._cognigy._default._image.imageAltText,
      language,
      translator,
      apikey
    );
  ...
  if (data?._data?._cognigy?._default?._image) {
    // Translate fallback text
    data._data._cognigy._default._image.fallbackText = await translate(
      data._data._cognigy._default._image.fallbackText,
      language,
      translator,
      apikey
    );
    // Translate alternative text
    data._data._cognigy._default._image.imageAltText = await translate(
      data._data._cognigy._default._image.imageAltText,
      language,
      translator,
      apikey
    );
...
const tokenizeText = (text: string): { tokens: string[]; text: string } => {
  ...
  const snippetMatches = text.match(
    /\[\[snippet\-(([a-f0-9]{8}\-[a-f0-9]{4}\-[a-f0-9]{4}\-[a-f0-9]{4}\-[a-f0-9]{12}\-)|([a-fA-F0-9]{24}\-)){0,1}[a-zA-Z0-9+\/]+={0,2}\]\]/g
  );
