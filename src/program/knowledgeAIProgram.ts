...
export const makeKnowledgeAIProgram = () => {
  ...
  knowledgeAI
    .name('knowledge-ai')
    .description(
      'CLI to manage data ingestion and removal of the knowledge AI.'
    );

  knowledgeAI.addHelpText(
    'after',
    `
Examples:
  ...
  knowledgeAI
    ...
    .description(
      `Creates knowledgeAI resources type [store, source] for a project with the specified name and description. Names must be unique.`
    )
...
Examples:
  ...
  knowledgeAI
    .command('ingest')
    .description(
      `Takes as "--input" a directory or ".txt" file. In case "--input" is a path to a directory, all ".txt" files 
    ...
    .option(
      '-n --name <string>',
      'Name of the knowledge store for file retrieval'
    )
    .option('-k, --knowledgeStoreId <string>', 'Knowledge Store ID')
    .requiredOption('-i, --input <path>', 'Path to file or directory')
    .action(async (options) => {
      try {
        await ingestCMD(options.knowledgeStoreId, options.input, options.name);
      } catch (e) {
        console.log(e.message);
      }
    })
...
Examples:
  ...
  knowledgeAI
    ...
    .description(
      `Converts "diffbot" output into text files parsing the output as paragraphs as expected as input for the "ingest" command.`
    )
    .option('-i, --inputFile <string>', 'Input File Path')
    .option('-o, --outputFile <string>', 'Output File Path')
    .option('-u --url <string>', 'Target URL (for web based extraction)')
    .option(
      '-e, --excludeString <string>',
      'Resulting paragraphs will be excluded if they contain this string'
    )
    .option(
      '-s, --splitter <string>',
