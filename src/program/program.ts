...
program
  ...
  .action(async (cmdObj) => {
    await clone({ resourceType: cmdObj.type, forceYes: cmdObj.forceYes });
  });
...
program
  ...
  .action(async (cmdObj) => {
    await restore({ resourceType: cmdObj.type, forceYes: cmdObj.forceYes });
  });
...
program
  ...
  .description(
    'Compares a local <resourceType> resource with <resourceId> to its corresponding remote resource'
  )
  .action(async (resourceType, resourceId, cmdObj) => {
    await diff(resourceType, resourceId, cmdObj.mode);
  });
...
program
  ...
  .option('-t, --timeout <ms>', 'timeout for training')
  .action(async (resourceType, resourceName, cmdObj) => {
    await train({ resourceName, timeout: cmdObj.timeout });
  });
...
program
  ...
  .action(
    async (
      resourceType,
      resourceName,
      resourceDescription = 'Cognigy.AI CLI',
      cmdObj
    ) => {
      await create({
        resourceType,
        ...
        skipDownload: cmdObj.skipDownload,
        fallbackLocale: cmdObj.fallbackLocale,
        nluLanguage: cmdObj.nluLanguage,
      });
    }
  );
...
program
  ...
  .action(async (resourceType, resourceName, cmdObj) => {
    await exportcsv({ resourceType, resourceName });
  });
...
program
  ...
  .action(async (resourceType, resourceName, cmdObj) => {
    await importcsv({ resourceType, resourceName });
  });
...
program
  ...
  .action(async (resourceType, resourceName, cmdObj) => {
    await localize({ resourceType, resourceName, options: cmdObj });
  });
...
program
  ...
  .option('-y, --forceYes', 'skips warnings and overwrites all content')
  .action(async (resourceType, resourceName, cmdObj) => {
    await translate({ resourceType, resourceName, options: cmdObj });
  });
