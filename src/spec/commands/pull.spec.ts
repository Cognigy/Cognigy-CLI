...
describe("Pull spec command", () => {
    ...
    describe("Pull Flow", () => {
        ...
        it("Should allow pulling flow resourceType", async () => {
            await pull({ resourceType: "flow", resourceName: "flow-name", forceYes: true });
    ...
    describe("Pull endpoint", () => {
        ...
        it("Should allow pulling endpoint resourceType", async () => {
            await pull({ resourceType: "endpoint", resourceName: "endpoint-name", forceYes: true });

            expect(pullEndPointSub.called).to.be.true;
            expect(pullEndPointSub.firstCall.args).to.be.deep.equal(["endpoint-name", 100]);
        });
    });
    ...
    describe("Pull lexicon", () => {
        ...
        it("Should allow pulling lexicon resourceType", async () => {
            await pull({ resourceType: "lexicon", resourceName: "lexicon-name", forceYes: true });

            expect(pullLexiconStub.called).to.be.true;
            expect(pullLexiconStub.firstCall.args).to.be.deep.equal(["lexicon-name", 100]);
        });
    });
    ...
    describe("Pull locales", () => {
        ...
        it("Should allow pulling locales resourceType", async () => {
            await pull({ resourceType: "locales", resourceName: "locales-name", forceYes: true });
    ...
    describe("Pull unrecognised resourceType", () => {
        it("Should throw an error", async () => {
            try {
                await pull({ resourceType: "wierd-resource-type", resourceName: "wierd-resource--name", forceYes: true });
                expect.fail();
            }
            catch (e) {
                expect(e, "Throws an Error").to.be.instanceOf(Error);
                expect(e.message, "Throws an Error").to.be.equal(`Resource type wierd-resource-type can't be pulled.`);
            }
