...
export const translate = async ({
  ...
  const { translator, apiKey, toLanguage, forceYes } = <ITranslateFlowOptions>(
    options
  );

  if (resourceType !== 'flow') {
    console.log(
      `\Translation of resource type ${resourceType} is not supported.`
    );
    process.exit(0);
  }
  ...
  if (!['google', 'microsoft', 'deepl'].includes(translator)) {
    console.log(
      "Please use 'google', 'microsoft' or 'deepl' as translation tool"
    );
    process.exit(0);
  }
